...
export default function Login() {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.unverified)       setErrorType("unverified");
        else if (res.status === 404) setErrorType("notfound");
        else if (res.status === 403) setErrorType("deactivated");
        else if (res.status === 401) setErrorType("invalid");
        else { setError(data.message || "Login failed."); setErrorType("generic"); }
        return;
      }
      setUser({
        token: data.token, role: data.role, name: data.name || "", email: data.email || email,
        avatar: data.avatar || null, userId: data.id, mqttTopic: data.mqttTopic || null,
      ...
      setError("Network error. Please try again."); setErrorType("generic");
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/api/auth/resend-verify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      else { setError(data.message || "Failed to resend verification email."); setErrorType("generic"); }
    } catch {
      setError("Network error. Please try again."); setErrorType("generic");
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      setError("Google login error. Please try again."); setErrorType("generic");
    }
  };
