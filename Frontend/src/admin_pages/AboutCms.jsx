...
const FIELDS = [
  ...
  { key: "body1",      label: "Body Paragraph 1",  textarea: true  },
  { key: "body2",      label: "Body Paragraph 2",  textarea: true  },
...
export default function AboutCms({ getHeaders, notify }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res  = await fetch(`${BASE}/api/cms?section=about`);
      const data = await res.json();
      setFields(data); setDraft({ ...data });
    } catch { notify("❌ Failed to load About content"); }
  ...
  const handleSave = async () => {
    ...
    try {
      const res  = await fetch(`${BASE}/api/cms`, {
        ...
        body: JSON.stringify({ section: "about", updates: draft }),
      });
      const data = await res.json();
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Edit the about section headline, body text and stat cards</p>
        </div>
        {dirty && (
